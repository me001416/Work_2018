...
namespace Combination
{
    ...
    public class Generate
    {
        ...
        /// <param name="k">深度，x 個數字中取 k </param>
        /// <returns>回傳所有 Combination</returns>
        public List<List<byte>> NewCombine(List<byte> SrcList, int k)
        {
            List<List<byte>> mResultList = new List<List<byte>>();

            if (SrcList.Count != 0 && k != 0 && SrcList.Count >= k)
            {
                List<byte> mTempList = new List<byte>();
                doCombin(0, SrcList.Count, k, mResultList, mTempList, SrcList);
            }
        ...
        /// <param name="mTempList">暫存用</param>
        /// <param name="mSrcList">資料來源</param>
        public void doCombin(int start, int end, int deep, List<List<byte>> mResultList, List<byte> mTempList, List<byte> mSrcList)
        {
            for (int i = start; i < end; i++)
            {
                if (end - i + 1 < deep)
                ...
                if (deep - 1 == 0)
                {
                    mResultList.Add(new List<byte>(mTempList));
                    mTempList.RemoveAt(mTempList.Count - 1);
                }
                else
                {
